...
The bundled HTML template in [`template/template_html/`](template/template_html/) is now the primary visual source for the skill workflow.

The legacy files in [`template/template_pptx/snupi_slide_template.pdf`](template/template_pptx/snupi_slide_template.pdf) and [`template/template_pptx/snupi_slide_template.pptx`](template/template_pptx/snupi_slide_template.pptx) are kept only as fallback visual reference.

The skills keep `slides-grab`'s native `720pt x 405pt` 16:9 frame and now try to reproduce the HTML template directly:
- large titles,
